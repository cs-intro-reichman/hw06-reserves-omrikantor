...
public class Runigram {
	...
	public static Color luminance(Color pixel) {
		int p = (int)((pixel.getRed() * 0.299) + (pixel.getGreen() * 0.587) + (pixel.getBlue() * 0.114));
		Color newPixel = new Color(p, p, p);
		return newPixel;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0; i < height; i++)
		{
			...
			{
				s[i][j] = image[(int) ((i * h) / height)][(int) ((j * w) / width)];
			}
		}
		return s;
	}
